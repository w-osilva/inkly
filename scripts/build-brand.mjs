...
// 2) Square transparent icons, centered.
for (const size of [16, 32, 48, 96, 128]) {
  execSync(
    `convert "${TMP}" -resize ${size}x${size} -background none -gravity center -extent ${size}x${size} "${ICON_DIR}/${size}.png"`,
  );
}
console.log('icons →', ICON_DIR, '(16/32/48/96/128)');
