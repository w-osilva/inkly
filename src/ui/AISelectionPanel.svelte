...
{#if aiPanelState.phase !== 'hidden'}
  ...
  >
    ...
    {:else if aiPanelState.phase === 'loading'}
      <!-- We never show partial output: live streaming just reflows/jank the panel (and would
           flash half-built JSON for synonyms/improve). Show the loader until the result is
           complete; the SSE still accumulates the final text under the hood. -->
