...
async function configure(context: import('@playwright/test').BrowserContext) {
  ...
  await sw.evaluate(async () => {
    ...
    await chrome.storage.sync.set({ 'inkly:settings': { globalEnabled: true, siteOverrides: {}, disabledCategories: [], dictionary: [], uiLanguage: 'pt-br' } });
  });
}
...
test('Translate action translates the selection to the UI language', async ({ context }) => {
  ...
  await expect(editor).toContainText('hello TRANSLATED[portuguese]: world');
});
