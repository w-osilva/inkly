...
test('the suggestion card shows a Portuguese category label when uiLanguage is pt-br', async ({ context }) => {
  ...
  await editor.click();
  await editor.type('I lielk cats'); // "lielk" → Spelling

  await hoverUntilCard(page);

  await expect(page.locator('css=.inkly-card__cat')).toHaveText('Ortografia', { timeout: 5_000 });
});
