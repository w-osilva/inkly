...
test('Synonyms shows alternatives; picking one replaces the selection', async ({ context }) => {
  ...
  await page.locator('css=.inkly-ai__btn').filter({ hasText: 'Synonyms' }).click();
...
test('Analyze shows read-only feedback with no Apply button', async ({ context }) => {
  await setAIConfig(context);
  const page = await context.newPage();
  await page.goto('/contenteditable.html');
  const editor = page.locator('#editor');
  await editor.click(); await editor.type('hello world');
  await selectWorld(page);
  await page.locator('css=.inkly-ai__btn').filter({ hasText: 'Analyze' }).click();
  await expect(page.locator('css=.inkly-ai__result')).toContainText('ANALYSIS:', { timeout: 10_000 });
