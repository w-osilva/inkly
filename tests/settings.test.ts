...
describe('isEnabledForHost', () => {
  const base: Settings = { globalEnabled: true, siteOverrides: {}, disabledCategories: [], dictionary: [], uiLanguage: 'auto', defaultTone: '', defaultStyles: [], defaultLength: 'asis', theme: 'auto' as const, correctionOrder: [], correctionDisabled: [], selectionActionsDisabled: [], languageToolEndpoint: 'https://api.languagetool.org/v2', languageToolPicky: false, autoAiCheck: false };
...
describe('getSettings/setSettings', () => {
  ...
  it('round-trips and merges over defaults', async () => {
    await setSettings({ globalEnabled: false, siteOverrides: { 'x.com': true }, disabledCategories: [], dictionary: [], uiLanguage: 'auto', defaultTone: '', defaultStyles: [], defaultLength: 'asis', theme: 'auto' as const, correctionOrder: [], correctionDisabled: [], selectionActionsDisabled: [], languageToolEndpoint: 'https://api.languagetool.org/v2', languageToolPicky: false, autoAiCheck: false });
...
describe('getSettings (malformed storage is normalized)', () => {
  ...
  it('migrates legacy autoSuggest/languageToolEnabled flags into correctionDisabled', async () => {
    await fakeBrowser.storage.sync.set({ 'inkly:settings': { autoSuggest: false, languageToolEnabled: false } });
    const s = await getSettings();
    expect(s.correctionDisabled).toEqual(expect.arrayContaining(['aiImprove', 'languagetool']));
  });
  it('does not migrate when the new correctionDisabled field is present', async () => {
    await fakeBrowser.storage.sync.set({ 'inkly:settings': { autoSuggest: false, correctionDisabled: [] } });
    expect((await getSettings()).correctionDisabled).toEqual([]);
  });
});
...
describe('category transforms', () => {
  const base = { globalEnabled: true, siteOverrides: {}, disabledCategories: [] as string[], dictionary: [] as string[], uiLanguage: 'auto' as const, defaultTone: '', defaultStyles: [], defaultLength: 'asis', theme: 'auto' as const, correctionOrder: [], correctionDisabled: [], selectionActionsDisabled: [], languageToolEndpoint: 'https://api.languagetool.org/v2', languageToolPicky: false, autoAiCheck: false };
...
describe('effectiveLang', () => {
  const base = { globalEnabled: true, siteOverrides: {}, disabledCategories: [], dictionary: [], defaultTone: '', defaultStyles: [], defaultLength: 'asis', theme: 'auto' as const, correctionOrder: [], correctionDisabled: [], selectionActionsDisabled: [], languageToolEndpoint: 'https://api.languagetool.org/v2', languageToolPicky: false, autoAiCheck: false };
